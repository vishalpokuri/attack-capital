...
export function UpdateBotModal({
  isOpen,
  onClose,
  onSubmit,
  bot,
  ...
  // Prefill form when bot data changes
  useEffect(() => {
    if (bot && isOpen) {
      setFormData({
        name: bot.name || "",
        ...
        voice_provider: bot.voice_provider || "Cartesia",
        voice: bot.voice || "",
        voice_speed: bot.voice_speed || 1,
        llm_model_name: bot.llm_model_name || "gpt-4.1",
        llm_model_temperature: bot.llm_model_temperature || 0.7,
        call_settings: {
          max_call_duration: bot.call_settings?.max_call_duration || 15,
          silence_timeout: bot.call_settings?.silence_timeout || 6,
          silence_timeout_max_retries:
            bot.call_settings?.silence_timeout_max_retries || 2,
          call_recording_enabled:
            bot.call_settings?.call_recording_enabled ?? true,
          voicemail_detection_enabled:
            bot.call_settings?.voicemail_detection_enabled ?? false,
          hipaa_compliance_enabled:
            bot.call_settings?.hipaa_compliance_enabled ?? false,
          pci_compliance_enabled:
            bot.call_settings?.pci_compliance_enabled ?? false,
        },
        advanced_settings: {
          agent_personality:
            bot.advanced_settings?.agent_personality || "friendly",
          humanize_conversation:
            bot.advanced_settings?.humanize_conversation ?? true,
          background_noise_reduction:
            bot.advanced_settings?.background_noise_reduction ?? true,
          allow_interruptions:
            bot.advanced_settings?.allow_interruptions ?? true,
          min_interruption_duration:
            bot.advanced_settings?.min_interruption_duration || 1,
          background_sound: bot.advanced_settings?.background_sound || "None",
          agent_response_length:
            bot.advanced_settings?.agent_response_length || "concise",
          short_pause: bot.advanced_settings?.short_pause || 1,
          long_pause: bot.advanced_settings?.long_pause || 4,
        },
        post_call_settings: {
          summary_prompt:
            bot.post_call_settings?.summary_prompt ||
            "You are an expert note-taker. You will be given a transcript of a call. Summarize the call in 2-3 sentences, if applicable.",
          success_evaluation_prompt:
            bot.post_call_settings?.success_evaluation_prompt ||
            "You are an expert evaluator. You will be given a transcript of a call. Evaluate the call based on the rubric provided. Respond with only the chosen option.",
          success_evaluation_rubric_type:
            bot.post_call_settings?.success_evaluation_rubric_type ||
            "PASS_FAIL",
        },
      });
    }
    ...
    <div className="fixed inset-0 bg-black/20 bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] m-4 overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">Update Bot</h2>
          <button
            onClick={onClose}
